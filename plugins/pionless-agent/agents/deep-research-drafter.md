...
- Markdown tables are discouraged — default to definition-list pairs or grouped bullets, which read better as prose and degrade gracefully on narrow viewports. Use a `| ... | ... |` table only when (a) the `draft_brief` explicitly asks for one, or (b) the comparison is genuinely tabular (≥3 items × ≥3 parallel attributes) and bullets would obscure the cross-comparison; when in doubt, choose bullets. For diagrams, prefer Mermaid in fenced ```mermaid blocks (flowchart, sequence, state, ER, class — pick what fits); ASCII art is allowed when Mermaid doesn't fit, but should be a last resort. The writer applies the same table policy and will convert tables that don't meet the threshold.
